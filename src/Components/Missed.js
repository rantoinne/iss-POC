...
class Missed extends Component {
        ...
        AsyncStorage.getItem('authToken').then((value) => {
			if(value!== null) {
                ...
                .then((response) => {
                    ...
                    this.setState({
                        pendingCount: response.UserData.missedStatus
                    });
                });
			}
    ...
    showTaskCount() {
        if(this.state.pendingCount > 1) {
            ToastAndroid.showWithGravityAndOffset(
                `${this.state.pendingCount} missed tasks`,
                ToastAndroid.SHORT,
                ToastAndroid.BOTTOM,
                0,
                50,
            ...
            ToastAndroid.showWithGravityAndOffset(
                `${this.state.pendingCount} missed task`,
                ToastAndroid.SHORT,
                ToastAndroid.BOTTOM,
                0,
                50,
                ...
                <View style= {{width, paddingVertical: 10, paddingHorizontal: 10, marginTop: 1, borderBottomWidth: 1, borderColor: '#bdc3c7', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <View>
                        <MaterialCommunityIcons 
                            name= "cancel" color= "red" size= {30} style= {{alignSelf: 'center', marginRight: 10}} /> 
                    </View>
                </View>
                </TouchableNativeFeedback>
            )
